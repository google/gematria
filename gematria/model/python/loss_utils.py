...
class LossComputation(tf.experimental.ExtensionType):
  ...
  def __init__(
    ...
    assert absolute_percentage_errors.shape.is_compatible_with(
        (num_tasks, None)
    )
    ...
    self.absolute_error_percentiles = self._make_percentile_tensor(
        absolute_errors, num_tasks, percentile_ranks, dtype
    )
    assert (
        not percentile_ranks
        or self.absolute_error_percentiles.shape.is_compatible_with(
            (len(percentile_ranks), num_tasks)
        )
    )
    self.absolute_percentage_error_percentiles = self._make_percentile_tensor(
        absolute_percentage_errors, num_tasks, percentile_ranks, dtype
    )
    assert (
        not percentile_ranks
        or self.absolute_percentage_error_percentiles.shape.is_compatible_with(
            (len(percentile_ranks), num_tasks)
        )
    )
    ...
    self.mean_absolute_error = self._loss_tensor(
        options.ErrorNormalization.NONE,
        options.LossType.MEAN_ABSOLUTE_ERROR,
        **tensor_args,
    )
    self.mean_squared_error = self._loss_tensor(
        options.ErrorNormalization.NONE,
        options.LossType.MEAN_SQUARED_ERROR,
        **tensor_args,
    )
    self.mean_absolute_percentage_error = self._loss_tensor(
        options.ErrorNormalization.PERCENTAGE_ERROR,
        options.LossType.MEAN_ABSOLUTE_ERROR,
        **tensor_args,
    )
    self.mean_squared_percentage_error = self._loss_tensor(
        options.ErrorNormalization.PERCENTAGE_ERROR,
        options.LossType.MEAN_SQUARED_ERROR,
        **tensor_args,
    )
    ...
    match loss_type:
      case options.LossType.MEAN_SQUARED_ERROR:
        tensor = tf.reduce_mean(
            self._squared_errors_witn_normalization(
                normalization,
                num_tasks,
                delta,
                squared_errors,
      ...
      case options.LossType.MEAN_ABSOLUTE_ERROR:
        tensor = tf.reduce_mean(
            self._absolute_errors_with_normalization(
                normalization,
                absolute_errors,
  ...
  def _squared_errors_witn_normalization(
    ...
    match normalization:
      ...
      case options.ErrorNormalization.EXPECTED_VALUE_GREATER_THAN_ONE:
        result = tf.square(delta / absolute_expected_outputs_or_one)
      case _:
  ...
  def _absolute_errors_with_normalization(
    ...
    match normalization:
      ...
      case options.ErrorNormalization.EXPECTED_VALUE_GREATER_THAN_ONE:
        return absolute_errors / absolute_expected_outputs_or_one
      case _:
        raise NotImplementedError(
