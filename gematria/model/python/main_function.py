...
def _task_names_match_throughput_source_filters(
    flags_dict: Mapping[str, Sequence[str]]) -> bool:
...
def _at_most_one_warm_start_source_can_be_used(
    flags_dict: Mapping[str, str]) -> bool:
...
def _warmstart_from_file(scaffold: tf.train.Scaffold, sess):
  ...
  training.partially_restore_from_checkpoint(_WARMSTART_FILE.value,
                                             _LOAD_GLOBAL_STEP_FROM_CKPT.value,
                                             sess)
...
def _warmstart_from_dir(scaffold: tf.train.Scaffold, sess):
  ...
  training.partially_restore_from_checkpoint(checkpoint,
                                             _LOAD_GLOBAL_STEP_FROM_CKPT.value,
                                             sess)
...
def _monitored_training_session_from_flags(
    ...
    is_chief: bool) -> tf.train.MonitoredTrainingSession:
...
def _resume_previous_experiment_if_needed():
  ...
  if not _RESUME_TO_DIR.value:
    raise ValueError('--gematria_resume_from_dir was provided without providing'
...
def _make_basic_block_reader_from_command_line_flags(
  ...
  if _ACTION.value != model_options.Action.PREDICT:
    ...
    if (_ACTION.value == model_options.Action.TRAIN or
  ...
  protos = tfrecord.read_protos(input_files,
    ...
    with tf.device(dev):
      ...
      with timer.scoped('Loading basic blocks'):
        if _ACTION.value not in model_options.ACTIONS_WITHOUT_INPUT_DATA:
          ...
          if not input_files:
            sys.exit('At least one .tfrecord file must be specified through'
                     ' --gematria_input_file.')
