...
class LossComputationTest(tf.test.TestCase):
  ...
  def test_single_task_ranking_softmax_loss(self):
    ...
    rank_loss_tensor = loss.loss_tensor(
        ...
        loss_type=options.LossType.RANKING_SOFTMAX_LOSS,
    )

    self.assertEqual(rank_loss_tensor.shape, (num_tasks,))
    with self.session() as sess:
  ...
  def test_multi_task_ranking_softmax_loss(self):
    num_tasks = 3
    actual_output = tf.placeholder(self.dtype, shape=(None, num_tasks))
    expected_output = tf.placeholder(self.dtype, shape=(None, num_tasks))
    ...
    with self.session() as sess:
      ...
      self.assertEqual(rank_loss[0], rank_loss[2])
      self.assertLess(rank_loss[1], rank_loss[0])
