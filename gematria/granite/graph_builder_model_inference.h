...
class GraphBuilderModelInference {
 ...
 private:
  ...
  // The mapping between input tensor names and their indices in the tflite
  // model. Allows name-based lookups of input tensors.
  std::unique_ptr<std::unordered_map<std::string_view, int>> input_name_to_idx_;
